...
func init() {
	...
	verify := &cobra.Command{
		Use:   "verify",
		Short: "Cross-check Apple's change history against adastra's local mutation log",
		Long: `Compares mutations recorded locally by adastra against Apple's change
...
another machine).`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(external) == 0 {
				fmt.Println("✓ no changes outside adastra detected")
				return nil
			}
