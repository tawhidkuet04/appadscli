...
func newCompetitorsCmd() *cobra.Command {
	...
	wAdd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("✓ watching", args[0], "— run `appadscli aso competitors watch run` to snapshot & diff")
			return nil
		},
	}
	...
	wRun := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(watched) == 0 {
				return fmt.Errorf("no competitors watched — `appadscli aso competitors watch add <adamId>` first")
			}
			type change struct {
