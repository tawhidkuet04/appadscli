...
func init() {
	...
	adjust.Flags().Float64Var(&targetROAS, "target-roas", 0, "target ROAS, e.g. 1.5 or 150 (needs `appadscli rc ingest`)")
...
Each rule runs the same engine as ` + "`appadscli bids adjust`" + ` scoped to its
