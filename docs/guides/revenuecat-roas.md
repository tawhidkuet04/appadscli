...
Apple's Ads API stops at installs. With the RevenueCat connector, appadscli joins
...
3. **Connect appadscli**:
   ```
   appadscli rc connect --api-key <RC_v2_secret_key> --project <projectId>
   ```
4. **Feed it data** — RevenueCat *Scheduled Data Exports* are the recommended
   ...
   ```
   appadscli rc ingest ./export.csv     # also accepts .jsonl
   ```
...
appadscli ltv report --by campaign --horizon 90d
```

## New optimization modes

```
appadscli bids adjust --adgroup <id> --target-roas 150% --dry-run   # bid to profitability
appadscli harvest run ... --rank-by roas                            # promote by revenue, not installs
```
